    ...
    try {
      ...
      content.innerHTML = `
        ...
        <div class="assignment-result-grid">
          ...
          <span><strong>${Number(result.calories)}kcal</strong>칼로리</span>
        ...
        <form id="trainerFeedbackForm" class="assignment-form trainer-feedback-form">
          ...
          <button type="submit">${result.trainer_feedback_id ? "피드백 수정" : "피드백 등록"}</button>
          <p id="trainerFeedbackMessage" class="assignment-form-message"></p>
        </form>`;
      ...
      content.innerHTML = `<p class="assignment-state">${escapeHtml(error.message)}</p>`;
    }
  }
