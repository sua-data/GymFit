...
class FakePoseModel:
    ...
    def __call__(self, *_args, **_kwargs):
        return self.predict(*_args, **_kwargs)
...
def test_down_stage_is_accepted_after_two_down_frames():
    analyzer = make_analyzer()

    update(analyzer, 95, frames=2)

    assert analyzer.stage == "DOWN"
    assert analyzer.count == 0
    assert analyzer.down_frames >= 2
...
def test_selected_arm_is_stable_across_short_confidence_fluctuations():
    ...
    for _ in range(4):
        assert (
            analyzer._extract_metrics(person, confidence)["selected_side"]
            == "LEFT"
        )
...
def test_process_contract_and_legacy_import(monkeypatch):
    ...
    assert status["debug"]["required_frames"] == 1
...
def test_coaching_session_reuses_same_analyzer_between_frames():
    store = CoachingSessionStore(
        analyzer_factory=lambda _code: make_analyzer()
    )
    ...
    with store.locked(session.session_id, user_id=1) as first_frame:
        update(first_frame.analyzer, 95, frames=2)
